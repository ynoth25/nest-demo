...
import { displayName, description, owner, link, tms, issue } from "allure-js-commons";

describe('TranslatorService', () => {
    let translatorService: TranslatorService;
    let translateUtil: TranslateUtil;
    let configService: ConfigService;
    let translatorRepository: Repository<TranslatorEntity>;

    beforeEach(async () => {
      const mockDataSource = {
        getRepository: jest.fn().mockReturnValue({
          createQueryBuilder: jest.fn().mockReturnValue({
            getMany: jest.fn().mockResolvedValue([]), // mock the response
          }),
        }),
      };

      const mockRepository = {
        find: jest.fn().mockResolvedValue([]), // You can add other methods as needed
      };
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
              ...
              {
                provide: 'usDataSource', // Mock DataSource for 'us'
                useValue: mockDataSource,
              },
              {
                provide: 'attDataSource', // Mock DataSource for 'att'
                useValue: mockDataSource,
              },
              {
                provide: getRepositoryToken(TranslatorEntity),
        ...
        translatorRepository = module.get<Repository<TranslatorEntity>>(getRepositoryToken(TranslatorEntity));
    });
    ...
    it('FAILED SR investigation and SQL script (No completed SR)', async () => {
      await displayName("Test Authentication");
      await displayName("Test Authentication");
      await description(
        "This test attempts to log into the website using a login and a password. Fails if any error happens.\n\nNote that this test does not test 2-Factor Authentication.",
      );
      await owner("John Doe");
      await link("https://dev.example.com/", "Website");
      await issue("AUTH-123", "");
      await tms("TMS-456", "http://");
